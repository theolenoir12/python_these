"""Pareto 25 ans -- FABLE -- leviers etat de sante : RB2(SoH_bat), RB2(SoH_H2), RB2(SoH_all).

Meme base que ../Predictions/Pareto_2d_25y_sohpred.py (baselines identiques,
socle cost-min, VoLL=3). Points ajoutes (runs deterministes, cf
sweep_fable_socwin_fine.txt et sweep_fable_unified.txt, notes note_rb2_soh_unifiee.txt) :
    RB2(SoH_bat) : plafond SoC vieillissant seul (g=0.2)   -> (2.7679, 56.842)
    RB2(SoH_H2)  : setpoints H2 x SoH (ex "RB2(SoH)")      -> (2.9089, 54.9115)
    RB2(SoH_all) : les deux leviers (ex "unifiee")         -> (3.1051, 52.870)
Deux versions :
    pareto_ems_fable.pdf/.png          : le Pareto (LPSP, deg).
    pareto_ems_fable_isocost.pdf/.png  : + lignes d'iso-cout unifie (VoLL=3).
...
# --- Points leviers SoH_bat (deterministes) ---
SOCMAX  = (2.7679, 56.842)   # plafond seul, g=0.2
UNIFIEE = (3.1051, 52.870)   # gammas (1,2) + plafond g=0.2
...
def build_figure(iso_cost):
    ...
    plt.tight_layout()
    stem = "pareto_ems_fable_isocost" if iso_cost else "pareto_ems_fable"
