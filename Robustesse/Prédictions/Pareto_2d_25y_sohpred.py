"""Pareto 25 ans -- PREDICTIONS -- EMS de base + RB2(SoH) + RB2(SoH+Pred).

RB2(SoH+Pred) = RB2(SoH) (socle cost-min 0.440*SoH_fc^1 / 0.310*SoH_ely^2) + pre-charge batterie previsionnelle,
...
Demarche SoH (2e temps) : on montre RB2(SoH) et son augmentation prevision.
Deux versions :
  - pareto_ems_sohpred.pdf/.png          : le Pareto (LPSP, deg).
  - pareto_ems_sohpred_isocost.pdf/.png  : + lignes d'ISO-COUT unifie.
...
RB2SOHP = (2.7534, 55.0846)   # (LPSP %, deg k€)
...
points = np.array([
    ...
    [2.9089, 54.9115],    # RB2(SoH)  (cost-min 0.440*SoH_fc^1 / 0.310*SoH_ely^2)
    [2.7534, 55.0846],        # RB2(SoH+Pred)  (re-optimise, moyenne MC)
    [1.2597, 80.1562],    # RB1
    [1.3389, 140.6745],   # SoC1
    [29.4642, 109.2535],  # SoC06
    [0.0000, 0.0000]      # Ideal
])
labels = ['0-100', '25-75', '50-50', '75-25', '100-0', 'RB2', 'RB2(SoH)', 'RB2(SoH+Pred)',
...
EXTRA_COLORS = {'RB2(SoH+Pred)': '#d95f02'}   # orange dedie, distinct du cluster
IDEAL_COLOR = '0.3'
...
def build_figure(iso_cost):
    ...
    zoom_labels = ['75-25', '100-0', 'RB2', 'RB2(SoH)', 'RB2(SoH+Pred)', 'RB1']
    ...
    zoom_offsets = {
        ...
        'RB2(SoH)': (0.12, -0.5, 'left', 'top'),
        'RB2(SoH+Pred)': (-0.10, 0.6, 'right', 'bottom'),
    ...
    plt.tight_layout()
    stem = "pareto_ems_sohpred_isocost" if iso_cost else "pareto_ems_sohpred"
