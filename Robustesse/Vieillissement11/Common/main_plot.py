...
def run_main_plot(data, start_timer=0, strategy_name=None):
    ...
    P_fc_max = fc_pmax(alpha_fc)
    P_ely_max = ely_pmax(alpha_ely)
    ...
    def plot_scientific_summary(temps, soh_d, deg_d, sdir):
        ...
        # Bilans de PREMIERE VIE pour l'encadre. Source robuste = lifetime_metrics
        # (data['first_life_metrics']) : le NaN insere sur le point pre-remplacement
        # du SoH casse la detection "maison" de la 1re vie, on ne la refait donc pas.
        flm = data.get("first_life_metrics", {})
        ...
        # hspace a 0.3 pour resserrer le tout verticalement
        ...
        for i, k in enumerate(['bat', 'fc', 'ely']):
            ...
            # 1. Extraction des donnees stridees
            ...
            # 2. Detection des sauts (remplacements)
            ...
            # On insere un point NaN juste apres chaque indice de saut
            ...
            # 4. Trace de la ligne (propre desormais)
            ax.plot(t_plot_clean, v_plot_clean, lw=6, color=colors[k], label=r'$SoH$', zorder=3)

            # Le fill_between peut rester sur les donnees originales stridees
            # car il gere generalement mieux les zones discontinues
        ...
        fc_hours   = flm.get('fc', {}).get('on_h', float('nan'))
        fc_energy  = flm.get('fc', {}).get('energy_kwh', float('nan'))
        ely_hours  = flm.get('ely', {}).get('on_h', float('nan'))
        ely_energy = flm.get('ely', {}).get('energy_kwh', float('nan'))

        for d, k, l, t, pos in cf_pie:
            ax = f.add_subplot(pos)
            # nanargmax : deg_d['total'] contient des NaN aux remplacements (cf. plus haut),
            # np.argmax tomberait dessus. On prend le pic de premiere vie fini.
            idx = max(0, int(np.nanargmax(d['total'])) - 1)
            v = np.array([d[key][idx] for key in k], dtype=float)
            m = np.isfinite(v) & (v > 0)
            if any(m):
                ...
                # Encadre reduit place a cote du camembert (sous la legende, a droite).
                ax.text(
                    0.95, 0.20,
                    txt,
        ...
        f.savefig(os.path.join(sdir, 'all_aging_2.pdf'), format='pdf', bbox_inches='tight')
        f.savefig(os.path.join(sdir, 'all_aging_2.png'), format='png', dpi=180, bbox_inches='tight')
        plt.close(f)
   ...
   # --- Courant et tension en fonction du temps : PEMFC et PEMWE ---
    i_fc_t = fc_current_from_power(np.abs(P_fc), alpha_fc)
    i_ely_t = ely_current_from_power(np.abs(P_ely), alpha_ely)
    ...
    i_fc_bol_max = float(fc_i_max(alpha_fc_bol))
    i_fc_mol_max = float(fc_i_max(alpha_fc_mol))
    i_fc_eol_max = float(fc_i_max(alpha_fc_eol))
    ...
    i_ely_bol_max   = float(ely_i_max(alpha_ely_bol))
    i_ely_mol_max   = float(ely_i_max(alpha_ely_mol))
    i_ely_eol_max   = float(ely_i_max(alpha_ely_eol))
