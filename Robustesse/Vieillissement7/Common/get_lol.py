...
def get_lol(SoC_t,action,P_tot_ref_t,defaillances,E_h2_t,E_h2_init,P_fc_max_t,P_ely_max_t,SoH_bat_t) :
        ...
        SoC_tp1 = SoC_t - P_bat_t * BAT['eff']**np.sign(-P_bat_t) / (BAT['parallel_num']*BAT['series_num']*BAT['Q_bat']*BAT['v_cell_nom']*SoH_bat_t) 
        
        if SoC_tp1 > 0.995 or SoC_tp1 < 0.2 :
            if SoC_tp1 > 0.995 :
                P_bat_t    = (SoC_t - 0.99499) * (BAT['parallel_num']*BAT['series_num']*BAT['Q_bat']*BAT['v_cell_nom']*SoH_bat_t) / BAT['eff']**np.sign(-P_bat_t)
                P_dc_bat_t = P_bat_t / CONV['eta']**np.sign(P_bat_t) 
                
            elif SoC_tp1 < 0.2 :
                P_bat_t   = (SoC_t - 0.20001) * (BAT['parallel_num']*BAT['series_num']*BAT['Q_bat']*BAT['v_cell_nom']*SoH_bat_t) / BAT['eff']**np.sign(-P_bat_t)
                P_dc_bat_t = P_bat_t / CONV['eta']**np.sign(P_bat_t) 
