...
def run_main_plot(data, start_timer=0, strategy_name=None):
    ...
    P_fc_max = i_fc_max * FC['n_parallel'] * FC['n_series'] * (FC['E_0'] - FC['R'] * (1 + alpha_fc) * i_fc_max / FC['n_parallel'] 
                                            - A * FC['T'] * np.log((i_fc_max / S / FC['n_parallel'] + j_in) / j_0)
                                            - B * FC['T'] * np.log(1 - i_fc_max / S / FC['n_parallel'] / j_L / (1 - alpha_fc)))
    ...
    P_ely_max = i_ely_max * ELY['n_parallel'] * ELY['n_series'] * (ELY['E_0'] + ELY['R'] * (1 + alpha_ely) * i_ely_max / ELY['n_parallel'] 
                                               + A * ELY['T'] * np.log((i_ely_max / S / ELY['n_parallel'] + j_in) / j_0)
                                               + B * ELY['T'] * np.log(1 - i_ely_max / S / ELY['n_parallel'] / j_L / (1 - alpha_ely)))
    ...
    for k in range(1,len(SoH_bat)) : 
        ...
        if deg_fc['total'][k] < 5e-3 and deg_fc['total'][k-1] > 1 :
            ...
            deg_fc['transient'][k-1]  = np.nan
            deg_fc['high'][k-1]        = np.nan
    ...
    def plot_degradation_pie_charts(deg_fc, deg_ely, target_dir):
        ...
        keys_fc = ['start-stop', 'idling', 'transient', 'high']
   ...
   # --- Courant et tension en fonction du temps : PEMFC et PEMWE ---
    ...
    def solve_i_fc(P_target, alpha):
        ...
        def residual(i):
            V = FC['n_series'] * (FC['E_0'] - FC['R'] * (1 + alpha) * i / FC['n_parallel']
                - A * FC['T'] * np.log((i / S / FC['n_parallel'] + j_in) / j_0)
                - B * FC['T'] * np.log(1 - i / S / FC['n_parallel'] / j_L / (1 - alpha)))
            return i * V - P_target
        try:
    ...
    def solve_i_ely(P_target, alpha):
        ...
        def residual(i):
            V = ELY['n_series'] * (ELY['E_0'] + ELY['R'] * (1 + alpha) * i / ELY['n_parallel']
                + A * ELY['T'] * np.log((i / S / ELY['n_parallel'] + j_in) / j_0)
                + B * ELY['T'] * np.log(1 - i / S / ELY['n_parallel'] / j_L / (1 - alpha)))
            return i * V - P_abs
        try:
            return brentq(residual, 1e-6, i_max * 0.999)
        except Exception:
    ...
    def voltage_fc_sweep(alpha, i_vec):
        ...
        return FC['n_series'] * (
            ...
            - A * FC['T'] * np.log((j_vec + j_in) / j_0)
            - B * FC['T'] * np.log(1 - j_vec / j_L / (1 - alpha))
        )
    ...
    def voltage_ely_sweep(alpha, i_vec):
        ...
        return ELY['n_series'] * (
            ...
            + A * ELY['T'] * np.log((j_vec + j_in) / j_0)
            + B * ELY['T'] * np.log(1 - j_vec / j_L / (1 - alpha))
        )
