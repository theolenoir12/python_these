...
# ---------------------------------------------------------------------------
def control_grid(n_fc=10, n_ely=50):
...
# ---------------------------------------------------------------------------
def battery_cost_step(soc, P_bat, soc_tp1):
    cu_t   = np.interp(soc,     deg_cumul1, deg_cumul2)[:, None]   # (Ns,1)
    cu_tp1 = np.interp(soc_tp1, deg_cumul1, deg_cumul2)            # (Ns,Nu)
    deg_SoC = np.abs(cu_t - cu_tp1) * (BAT['Q_bat'] * 1.0) * BAT['parallel_num'] / 2.15
    i_bat  = P_bat / V_SER                                         # (Nu,)
    C_rate = np.abs(i_bat) / (BAT['Q_bat'] * 1.0 * BAT['parallel_num'])
...
# ---------------------------------------------------------------------------
def soc_step_and_lpsp(soc, u, P_tot_ref):
...
def backward(soc_grid, h2_grid, u, pre, P_ref, VT, store_policy=False):
    ...
    for t in range(len(P_ref) - 1, -1, -1):
        Ptot = P_ref[t]
        soc_clamp, P_bat, lpsp = soc_step_and_lpsp(soc_grid, u, Ptot)  # (Ns,Nu),(Nu,),(Ns,Nu)
        cbat = battery_cost_step(soc_grid, P_bat, soc_clamp)           # (Ns,Nu)
