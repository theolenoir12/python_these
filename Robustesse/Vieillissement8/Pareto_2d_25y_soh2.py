...
NE REMPLACE PAS les fichiers existants : derive de Pareto_2d_25y.py, ecrit dans
pareto_ems_soh2.pdf/.png. Les autres EMS gardent leurs valeurs de reference
(batch_results_summary_25y.txt, metrique identique au sweep : RB2 plain concorde
a 1e-3). Seul le point RB2(SoH) est mis a jour."""
import numpy as np
...
plt.tight_layout()
plt.savefig("pareto_ems_soh2.pdf", format='pdf', bbox_inches='tight')
plt.savefig("pareto_ems_soh2.png", format='png', dpi=150, bbox_inches='tight')
plt.show()
print("saved -> pareto_ems_soh2.pdf / .png")
