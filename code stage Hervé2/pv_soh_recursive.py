...
datapro = df.iloc[:,0].values.reshape(-1, 1)
...
train_end = int(n * 0.70)        # étape de validation pour trouver les hyperparamètres
test_end  = int(n * 0.85)

# train_end = int(n * 0.85)     # étape de test
# test_end  = int(n * 1)
...
plt.ylabel("Energie de Production (Wh)")
plt.legend()
...
plt.xlabel("Heures de prédiction")
plt.ylabel("Energie de Production (Wh)")
plt.legend()
plt.grid(True)
plt.show()
...
ax2.set_ylabel("Energie de Production (Wh)")
ax2.legend()
